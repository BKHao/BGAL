...
bool _Polygon::is_in_(const _Point2 &in_p) const {
  ...
  for (int i = 0; i < _points.size(); ++i) {
    ...
    if (_BOC::sign_((__p1 - in_p).cross_(__p2 - in_p).length_()) == _BOC::_Sign::ZerO
        && (_BOC::sign_((__p1 - in_p).dot_((__p2 - in_p))) == _BOC::_Sign::NegativE
    ...
    if (((_BOC::sign_(__p1.y() - in_p.y()) == _BOC::_Sign::PositivE)
        ...
        && _BOC::sign_(in_p.x() - (in_p.y() - __p1.y()) * (__p1.x() - __p2.x()) / (__p1.y() - __p2.y()) - __p1.x())
            == _BOC::_Sign::NegativE) {
      __res = !__res;
    }
  }
...
_Point2 _Polygon::nearest_point_(const _Point2 &in_p) {
  ...
  for (int i = 0; i < num_(); ++i) {
    ...
    if (len < 0 || len > (_points[(i + 1) % num_()] - _points[i]).sqlength_()) {
      ...
      lp = _points[i]
          + (_points[(i + 1) % num_()] - _points[i]) * len / ((_points[(i + 1) % num_()] - _points[i]).sqlength_());
      len = (lp - in_p).length_();
    }
    if (len < min_dis) {
      min_dis = len;
...
int _Polygon::intersection_with_linesegment_(const _Point2 &p1,
                                             const _Point2 &p2,
  ...
  for (int i = 0; i < _points.size(); ++i) {
    ...
    if (_BOC::sign_((_points[i] - p1).dot_(_points[(i + 1) % _points.size()] - p1)
                        + ((_points[i] - p1).length_()) * ((_points[(i + 1) % _points.size()] - p1).length_()))
        == _BOC::_Sign::ZerO) {
      p1_on = true;
    }
    ...
    if (_BOC::sign_((p1 - _points[(i + 1) % _points.size()]).dot_(p2 - _points[(i + 1) % _points.size()])
                        + ((p1 - _points[(i + 1) % _points.size()]).length_())
                            * ((p2 - _points[(i + 1) % _points.size()]).length_())) == _BOC::_Sign::ZerO) {
      continue;
    }
