...
namespace BKHao
{
	...
	_BOC::_Sign _Restricted_Tessellation3D::side_(const int& ip1, const int& ip2, _Symbolic_Point& v)
	{
		...
		case 3:
		{
			...
			res = _Side3D::side3_(p1.x(), p1.y(), p1.z(), p2.x(), p2.y(), p2.z(), p3.x(), p3.y(), p3.z(), p4.x(), p4.y(), p4.z(), _weights[ip1], _weights[ip2], _weights[ip3], _weights[ip4], q1.x(), q1.y(), q1.z(), q2.x(), q2.y(), q2.z(), q3.x(), q3.y(), q3.z());
			break;
		}
		default:
	...
	_Restricted_Tessellation3D::_Symbolic_Point _Restricted_Tessellation3D::insec_bisector_(const _Symbolic_Point& p1, const _Symbolic_Point& p2, const int& neigh, const int& center, const std::vector<int>& current_face_vid) const
	{
	...
	void _Restricted_Tessellation3D::calculate_()
	{
		...
		wps.push_back(std::make_pair(Weighted_point(Point(3 * min_p.x() - 2 * max_p.x(), 3 * min_p.y() - 2 * max_p.y(), 3 * min_p.z() - 2 * max_p.z()), min_weight), -1));
		wps.push_back(std::make_pair(Weighted_point(Point(3 * max_p.x() - 2 * min_p.x(), 3 * min_p.y() - 2 * max_p.y(), 3 * min_p.z() - 2 * max_p.z()), min_weight), -1));
		wps.push_back(std::make_pair(Weighted_point(Point(3 * max_p.x() - 2 * min_p.x(), 3 * max_p.y() - 2 * min_p.y(), 3 * min_p.z() - 2 * max_p.z()), min_weight), -1));
		wps.push_back(std::make_pair(Weighted_point(Point(3 * min_p.x() - 2 * max_p.x(), 3 * max_p.y() - 2 * min_p.y(), 3 * min_p.z() - 2 * max_p.z()), min_weight), -1));
		wps.push_back(std::make_pair(Weighted_point(Point(3 * min_p.x() - 2 * max_p.x(), 3 * min_p.y() - 2 * max_p.y(), 3 * max_p.z() - 2 * min_p.z()), min_weight), -1));
		wps.push_back(std::make_pair(Weighted_point(Point(3 * max_p.x() - 2 * min_p.x(), 3 * min_p.y() - 2 * max_p.y(), 3 * max_p.z() - 2 * min_p.z()), min_weight), -1));
		wps.push_back(std::make_pair(Weighted_point(Point(3 * max_p.x() - 2 * min_p.x(), 3 * max_p.y() - 2 * min_p.y(), 3 * max_p.z() - 2 * min_p.z()), min_weight), -1));
		wps.push_back(std::make_pair(Weighted_point(Point(3 * min_p.x() - 2 * max_p.x(), 3 * max_p.y() - 2 * min_p.y(), 3 * max_p.z() - 2 * min_p.z()), min_weight), -1));
	...
	_Restricted_Tessellation3D::_Restricted_Tessellation3D(const _ManifoldModel& in_model, const std::vector<_Point3>& in_sites)
		:_num_sites(in_sites.size()),
		_sites(in_sites),
		_model(in_model)
	{
