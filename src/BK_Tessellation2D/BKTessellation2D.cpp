...
BKHao::_Tessellation2D::_Tessellation2D(const _Polygon &in_boundary,
...
void BKHao::_Tessellation2D::calculate_(const _Polygon &in_boundary,
                                        const std::vector<_Point2> &in_sites,
                                        const std::vector<double> &in_weights) {
  _boundary = in_boundary;
  _sites = in_sites;
...
BKHao::_BOC::_Sign BKHao::_Tessellation2D::symbolic_point_site_(const int &site1,
                                                                const int &site2,
  ...
  while (!Qse.empty()) {
    ...
    for (auto it = _adjacent_edges[cur_site].begin(); it != _adjacent_edges[cur_site].end(); ++it) {
      ...
      if (side_s == _BOC::_Sign::NegativE && side_t == _BOC::_Sign::PositivE) {
        ...
        if ((_boundary[(cur_edge + 1) % (_boundary.num_())] - _boundary[cur_edge]).cross_(
            _sites[cur_site] - _sites[it->first]).z() < 0) {
          sym_ps = std::make_pair(0, 0);
    ...
    if (sym_ps.first == 0) {
      ...
      if (sym_pt.first < 0 && sym_pt.second < 0) {
        if (site_edge_pair_is_visited.find(std::make_pair(cur_site, (cur_edge + 1) % (_boundary.num_())))
            == site_edge_pair_is_visited.end()) {
