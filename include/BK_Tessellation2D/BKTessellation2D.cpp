...
BKHao::_Tessellation2D::_Tessellation2D(const _Polygon& in_boundary, const std::vector<_Point2>& in_sites)
...
BKHao::_Tessellation2D::_Tessellation2D(const _Polygon& in_boundary, const std::vector<_Point2>& in_sites, const std::vector<double>& in_weights)
...
void BKHao::_Tessellation2D::calculate_(const _Polygon& in_boundary, const std::vector<_Point2>& in_sites)
{
...
void BKHao::_Tessellation2D::calculate_(const _Polygon& in_boundary, const std::vector<_Point2>& in_sites, const std::vector<double>& in_weights)
{
	_boundary = in_boundary;
	_sites = in_sites;
	_num_sites = in_sites.size();
...
const std::vector<BKHao::_Polygon>& BKHao::_Tessellation2D::get_cell_polygons_() const
{
	return _cell_polygons;
}

const std::vector<std::vector<std::pair<int, int>>>& BKHao::_Tessellation2D::get_cells_() const
{
	return _cells;
}

const BKHao::_Point2& BKHao::_Tessellation2D::vertex_(const int& id) const
{
...
int BKHao::_Tessellation2D::num_hidden_() const
{
	int count = 0;
...
BKHao::_BOC::_Sign BKHao::_Tessellation2D::symbolic_point_site_(const int& site1, const int& site2, const std::pair<int, int>& symp, const int& cur_edge)
{
	_BOC::_Sign r;
	...
	if (symp.first < 0 && symp.second < 0)
	{
		if (symp.first - symp.second == -1)
			r = _Side2D::side1_(_sites[site1].x(), _sites[site1].y(), _weights[site1], _sites[site2].x(), _sites[site2].y(), _weights[site2], _boundary[-symp.first - 1].x(), _boundary[-symp.first - 1].y());
		else if (symp.first - symp.second == 1)
			r = _Side2D::side1_(_sites[site1].x(), _sites[site1].y(), _weights[site1], _sites[site2].x(), _sites[site2].y(), _weights[site2], _boundary[-symp.second - 1].x(), _boundary[-symp.second - 1].y());
		else
			r = _Side2D::side1_(_sites[site1].x(), _sites[site1].y(), _weights[site1], _sites[site2].x(), _sites[site2].y(), _weights[site2], _boundary[0].x(), _boundary[0].y());
	}
	else
	{
		int site3 = symp.first > 0 ? symp.first - 1 : symp.second - 1;
		r = _Side2D::side2_(_sites[site1].x(), _sites[site1].y(), _weights[site1], _sites[site2].x(), _sites[site2].y(), _weights[site2], _sites[site3].x(), _sites[site3].y(), _weights[site3], _boundary[cur_edge].x(), _boundary[cur_edge].y(), _boundary[(cur_edge + 1) % (_boundary.num_())].x(), _boundary[(cur_edge + 1) % (_boundary.num_())].y());
	}
	return r;
}

BKHao::_Point2 BKHao::_Tessellation2D::convert_sym_to_point_(const std::set<int>& sym)
{
	std::vector<int> s;
	std::vector<int> e;
...
void BKHao::_Tessellation2D::calculate_()
{
	...
	while (!Qse.empty())
	{
		...
		Qse.pop();
		std::pair<int, int> sym_ps = std::make_pair(-(cur_edge + _boundary.num_() - 1) % (_boundary.num_()) - 1, -cur_edge - 1);
		...
		else
		{
			if (sym_ps.first < 0 && sym_ps.second < 0)
			{
				if (site_edge_pair_is_visited.find(std::make_pair(cur_site, (cur_edge + _boundary.num_() - 1) % (_boundary.num_()))) == site_edge_pair_is_visited.end())
				{
					site_edge_pair_is_visited.insert(std::make_pair(cur_site, (cur_edge + _boundary.num_() - 1) % (_boundary.num_())));
			...
			for (int j = 0; j < boundary_cross[i].size(); ++j)
			{
				if (boundary_cross[i][j].second == 0 || boundary_cross[i][j].second == -2)
				{
					if (boundary_cross[i][j].second == -2)
					{
						...
						std::pair<int, int> __new_v = std::make_pair(-(__bv + _boundary.num_() - 1) % (_boundary.num_()) - 1, -__bv - 1);
	...
	while (!Qss.empty())
	{
		...
		do
		{
			...
			for (int i = 0; i < boundary_cross[cur_site].size(); ++i)
			{
				if ((boundary_cross[cur_site][i].second == 1 || boundary_cross[cur_site][i].second == -1)
					&& (boundary_cross[cur_site][i].first.first == adj_site || boundary_cross[cur_site][i].first.second == adj_site))
				{
